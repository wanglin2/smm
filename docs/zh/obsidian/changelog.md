...
修复：
1.修复选择主题列表中最后一个主题时没有选中样式的问题；
2.修复转换为md、导出为md、复制为md时节点文本中插入的[[xxx]]内链丢失，变成纯文本的问题；
3.修复打开了思维导图标签页时页面所有输入框中鼠标点击文本选中区域没有响应的问题（正常交互为取消选中状态，光标定位至鼠标单击的两个文字之间）；

新增：
1.支持在上方插入同级节点（快捷键Shift+Enter，节点右键菜单）；
2.支持插入块级数学公式。1.侧边栏增加勾选框；2.文本编辑时输入$$xxx$$；

## 0.2.8

新增：
1.新增彩虹系列主题；
2.主题列表中主题名称改为鼠标移上显示、不再显示无意义的主题名称；
3.全屏大纲支持显示节点图片；
4.登录时端口被占用进行提示；
5.点击思维导图标签顶部的标题交互优化：第一次点击全选中标题文字，第二次点击取消全选并让光标处于鼠标点击位置；
6.转换md为思维导图、导入md时，默认不创建为自由节点，改为可选项；
...
修复：
1.修复节点链接的文件名称修改后节点显示没有更新的问题；

新增：
1.移动端顶部工具栏增加进入节点文本编辑的按钮；
2.节点富文本悬浮工具栏根据窗口大小进行动态适配，优化小屏幕显示不全的问题；
3.点击专业版功能时的未激活提醒信息支持跳转到相关链接；
...
修复：
1.修复移动端ios系统点击登录无反应的问题（ios增加二次跳转确认）；
2.修复在移动端ios系统上右键菜单进入的填空模式显示异常的问题（ios去掉下划线显示）；
3.修复安装了Exclidraw插件后Ctrl+S快捷键不生效、不显示【进入演示模式】和【保存并更新图像】命令的问题；

新增：
1.插件设置-顶部快捷跳转根据设置窗口宽度进行动态适应；
2.保存并更新预览图像命令的默认快捷键调整为Ctrl+Shift+M；

专业版：
1.节点右键菜单新增【编号节点文本】菜单；

## 0.2.5

修复：
1.尝试修复移动端ios系统点击登录无反应的问题；
2.尝试修复在移动端ios系统上右键菜单进入的填空模式显示异常的问题；
3.修复进入指定节点编辑后没有触发保存就退出节点时编辑内容丢失的问题；
4.修复移动端进入禅模式后无法退出的问题；
5.移动端适配：修复节点富文本悬浮工具栏显示不完全的问题；
6.修复预览Markdown为思维导图时，Markdown中引用的仓库图片在思维导图中加载失败的问题；
7.修复插件侧边栏-基础样式配置导入导出部分属性不生效的问题；
8.修复从文件列表中拖拽图片到节点图片添加弹窗中，点击确定添加无反应的问题；
9.修复可以拖拽非图片文件到节点图片添加弹窗中的问题；

新增：
1.将Markdown预览为思维导图、将Markdown转换为思维导图：支持解析![[]]格式插入的图片文件；
2.支持从指定模板创建：可在设置中指定单个模板文件或模板文件夹；
3.插件侧边栏-基础样式支持一键恢复默认配置；
4.插件侧边栏-设置：设置一键恢复为默认配置；
5.进入禅模式、填空模式后左上角新增退出按钮；
...
修复：
1.修复解析带转义%的数学公式的markdown内容时丢失转义，导致转换为思维导图后数学公式渲染失败的问题；
2.修复导入md文件时，当没有勾选转换数学公式选项时，ob内链也不会被转换的问题；
3.尝试修复移动端ios系统点击登录无反应的问题；
4.修复ob1.13版本更新后设置顶部快速跳转功能失效的问题；

新增：
1.转换md为思维导图、导入md时支持识别并保留文本的换行；
2.AI生成时支持解析和保留换行、支持渲染数学公式；
3.登录时端口被占用时自动尝试下一个端口；
4.思维导图搜索支持选项：区分大小写、全字匹配、使用正则表达式；
5.大纲编辑模式支持搜索；
6.大纲编辑模式插入新节点时自动滚动到新节点；
7.大纲编辑模式中的ctrl+v、ctrl+enter、ctrl+f等快捷键支持mac系统的command键；
...
修复：
1.尝试修复在ios系统上点击登陆没有反应的问题；
2.修复同时打开多个标签页，在一个标签页里开启了填空模式会影响其他标签页的问题；
3.修复节点插入的图片保存到源文件的# linkdata区块中时不是以![[]]格式的问题；
4.新标签页预览md为思维导图，修复在思维导图中使用搜索会显示替换按钮的问题；
5.修复激活根节点没有禁用顶部工具栏的外框按钮的问题；
6.修复部分AI返回`<think>思考内容</think>正文内容`格式数据导致解析失败的问题；
7.修复AI流式生成时偶现的某次响应为非合法json导致生成失败的问题；
8.修复在节点文本插入[[xxx|xxx]]格式的内链，点击链接会报错的问题；

新增：
1.预览Markdown文档时为思维导图时，如果解析出多个根节点，那么额外添加一个根节点，以当前文件名称为根节点的文本；
2.节点右键菜单中增加【在该节点下搜索】菜单；
3.在插件设置中修改语言选项后重载设置页；
4.插件设置：目录相关的设置增加提示信息：仅对后续新插入的文件生效，原有文件不会自动移动；
5.插件AI设置，填写了api和key后自动获取可用模型列表；
6.对于不支持跨域请求的AI接口自动转为非流式请求模式；
7.md转换为思维导图、导入md时自动转换其中的[[]]格式链接；
8.从ob文件列表或电脑本地拖拽文件到节点上，以[[]]链接格式添加到节点文本中，不再以超链接图标形式；
9.顶部工具栏【超链接】按钮打开的弹窗中移除添加【Obsidian文件】、【本地文件】能力，仅支持添加超链接；
10.在思维导图中插入的图片、文件文件名或位置改变后，同步更新思维导图中的引用；
11.【预览为思维导图】的ob命令执行后当没有预览是显示预览，有预览时关闭预览；

## 0.2.2

修复：
1.修复没有激活节点时，选择导入为当前激活节点下级时，loading不会关闭的问题；
2.修复开启演示模式的填空模式不会实时生效，需要重新打开窗口才会生效的问题；
3.修复给节点添加普通超链接时，再次回显时协议回显不正确的问题；
4.优化在节点文本中输入[[触发的文件列表弹窗的位置显示；
5.修复在Modal Opener插件的模态框中不响应快捷键的问题；
6.修复部分用户打开为Markdown文档操作失败的问题；
7.修复AI配置完成后，点击【测试】提示配置未完成的问题；
8.修复自由节点树的节点文本中添加的链接不会保存到源文件的# linkdata区块中的问题；
9.修复开启【支持ob搜索】设置，自由节点树的节点文本中没有保存到源文件的# textdata区块中的问题；

新增：
1.思维导图编辑标签页顶部的文件名输入窗口点击默认选中.smm扩展名之前的部分；
2.当没有激活任何节点时按F2快捷键进入当前导图文件重命名；
3.给节点添加普通超链接时，适配非http/https的自定义协议；
4.插件设置中增加切换数学公式渲染模式的设置；
5.在节点文本中输入[[时支持以[[后的文字作为搜索项过滤出文件列表，同时去除文件列表中的搜索框；
6.增加【预览为思维导图】的ob命令；
7.思维导图和Markdown互相转换支持以新建文件的方式进行转换；
8.适配节点文本中通过[[]]插入xxx://自定义协议的链接；
