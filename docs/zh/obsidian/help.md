...
- Markdown和思维导图互相转换，能否无损切换？

不能，会丢失无法部分数据。

- 同样的Markdown数据，预览为思维导图时不会重叠，而转换为思维导图会显示为多个根节点，并重叠，是不是bug？

不是，设计如此，解析到多个根节点时，编辑模式会创建为自由节点，发生重叠了可以手动拖动自由节点的根节点，预览模式因为无法拖动所以添加了一个根节点来包裹。

- 思维导图编辑窗口右上角为什么没有导入、导出、保存等工具按钮？
...
大概率是网络问题导致插件文件没有下载成功，建议删除重新下载，最好翻墙后下载。
